<?php
class ModelCheckoutInvoices extends Model {

        public function addVm($data, $product_id){
                //get all defined subdomain names (for each product in order
                $productoptions=$this->db->query("SELECT * FROM `" . DB_PREFIX . "order_option`  WHERE product_option_value_id = '0' AND order_id = '" . (int)$data['order_id']. "'");
                    $subdomain = $productoptions->row['value'];  
                    $this->db->query("INSERT INTO `" . DB_PREFIX . "vms` SET userID = '" . $this->db->escape($data['customer_id']) . "',  order_id = '" . (int)$data['order_id'] . "', vmName = '" . $subdomain . "', status = 'PEN'");

                    $vm_id = $this->db->getLastId();
	...
	public function getInvoice($invoice_id) {
                ...
                $invoice_query=$this->db->query("SELECT * FROM `" . DB_PREFIX . "cycling_invoices`  WHERE invoice_id = '" . (int)$invoice_id . "'");

                if ($invoice_query->num_rows) {
                  $order_id=$invoice_query->row["oreder_id"];
                }
		...
		if ($order_query->num_rows) {
			...
			return array(
                                ...
                                'txnid'                   => $invoice_query->row['txnid'],
                                'status_id'               => $invoice_query->row['status_id'],
                                'amount'                  => $invoice_query->row['amount'],
                                'order_id'              => $invoice_query->row['order_id'],
                                'date_added'              => $invoice_query->row['date_added'],
                                'datePayed'              => $invoice_query->row['datePayed'],
                                'dateExpire'              => $invoice_query->row['dateExpire'],
                                'factPeriod'              => $invoice_query->row['factPeriod'],
                              
				'order_id'                => $invoice_query->row['order_id'],

                                //Data from order query
